---
title: Build a not boring, accessible audio player with React
date: 2023-12-23
...
Seeking the audio is where things start to get a little more complicated because we being to introduce some accessibility concerns. The gist of seeking an audio track is to allow users to pick a specific time within the range of the track to go to. I’ve built many audio players in the past that use the mouse location to calculate a percentage to apply the audio time. Something like this:
...
This more-or-less works, but does not take accessibility into account. There’s no way to seek with just the keyboard. As far as I can tell, a more accessible way to achieve this is use the `range` input type instead. Using a native input control is generally a good idea for accessibility and it also gives us a lot of the seeking logic for free. 
